...
const ResumeDocument = () => {
  ...
  return (
    ...
    >
      <header className={`${block} bg-[#0f172a] text-white px-8 py-6 print:py-5 border-b-4 border-[#0d9488]`}>
      ...
      <div className="px-8 py-5 print:px-6 print:py-4">
        <ResumeSection title={labels.professionalSummary}>
          <p className={`${block} text-[#1e293b] text-[14px] leading-[1.75] font-bold`}>
        ...
        <ResumeSection title={labels.professionalExperience}>
          {experience.map((job) => (
            <div key={job.role} className={`${block} mb-3 last:mb-0 pb-3 last:pb-0 border-b border-[#e2e8f0] last:border-b-0`}>
        ...
        <ResumeSection title={labels.education}>
          {education.map((edu) => (
            <div key={edu.degree} className={`${block} mb-3 last:mb-0 pb-3 last:pb-0 border-b border-[#e2e8f0] last:border-b-0`}>
              <div className="flex flex-wrap justify-between items-start gap-x-3 gap-y-1">
                <h3 className="font-black text-[#0f172a] text-[14px] leading-snug pr-2">{edu.degree}</h3>
                <span className="text-[11.5px] text-[#0f172a] font-black whitespace-nowrap shrink-0">{edu.period}</span>
              </div>
        ...
        <ResumeSection title={labels.certifications}>
          <div className="space-y-2">
            {featuredCerts.map((cert) => (
              <div key={cert.name} className={`${block} pb-2 border-b border-[#f1f5f9] last:border-b-0 last:pb-0`}>
          ...
          {otherCerts.length > 0 && (
            <p className={`${block} text-[11.5px] text-[#475569] mt-3 leading-[1.6] font-semibold`}>
        ...
        <div className="cv-footer-grid border-t border-[#e2e8f0] pt-4 mt-2">
          <ResumeSection title={labels.technicalSkills} className="cv-footer-skills">
            <div className="grid grid-cols-2 gap-x-4 gap-y-3">
              {Object.entries(skills).map(([group, items]) => (
                <div key={group} className={block}>
                  <p className="text-[10.5px] font-black uppercase tracking-[0.1em] text-[#0f172a] mb-1.5">
          ...
          <div className="cv-footer-meta grid grid-cols-2 gap-4 mt-1">
            <ResumeSection title={labels.languages}>
              <ul className="space-y-2">
                {spokenLanguages.map((lang) => (
                  <li key={lang.name} className={`${block} border-b border-[#f1f5f9] pb-2 last:border-b-0 last:pb-0`}>
            ...
            <ResumeSection title={labels.references}>
              <ul className="space-y-2.5">
                {references.map((ref) => (
                  <li key={ref.name} className={`${block} border-b border-[#f1f5f9] pb-2 last:border-b-0 last:pb-0`}>
